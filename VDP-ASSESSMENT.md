...
- `README.md` describes running through the Monitor (`G 0800`, `J 0800`) and ending with
  "`BRK` into the Monitor", alongside `SYS 2048` and Wozmon `800R`.
...
**Part 2 is blocked on 6502-ASM's convention.** The work mirrors 6502-PRG's.
...
3. `README.md`:
   - A VDP binary needs BIOS 2.x and a converted ACE, and can check `KernalVersion` and
     the card-type byte at entry.
   - **Loading without the Monitor (2.x):** `BLOAD addr,"name"` from CF or `BLOAD addr`
     over XModem, then `SYS addr`. Or paste the `.woz` into Wozmon, which stays.
   - The Monitor rows (`G`/`J`) become 1.x only. On 2.x a `BRK` prints the one-line
     report and returns to BASIC, which is a reasonable way for a binary to stop.
